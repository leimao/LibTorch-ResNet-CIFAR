...
struct BasicBlock : torch::nn::Module {
  BasicBlock(int64_t inplanes, int64_t planes, int64_t stride = 1,
    ...
    m_conv1 = register_module("conv1", torch::nn::Conv2d{create_conv3x3_options(
...
struct Bottleneck : torch::nn::Module {
  Bottleneck(int64_t inplanes, int64_t planes, int64_t stride = 1,
    ...
    m_conv1 = register_module(
        "conv1", torch::nn::Conv2d{create_conv1x1_options(inplanes, width)});
    m_bn1 = register_module("bn1", torch::nn::BatchNorm2d{width});
    m_conv2 =
        register_module("conv2", torch::nn::Conv2d{create_conv3x3_options(
                                     width, width, stride, groups, dilation)});
    m_bn2 = register_module("bn2", torch::nn::BatchNorm2d{width});
    m_conv3 = register_module("conv3", torch::nn::Conv2d{create_conv1x1_options(
                                           width, planes * m_expansion)});
    m_bn3 =
        register_module("bn3", torch::nn::BatchNorm2d{planes * m_expansion});
    m_relu = register_module("relu", torch::nn::ReLU{true});
    if (!downsample->is_empty()) {
      m_downsample = register_module("downsample", downsample);
    }
...
template <typename Block> struct ResNet : torch::nn::Module {
  ResNet(const std::vector<int64_t> layers, int64_t num_classes = 1000,
    ...
    m_conv1 = register_module(
        "conv1", torch::nn::Conv2d{create_conv_options(
    ...
    m_layer2 = register_module(
        "layer2",
        _make_layer(128, layers.at(1), 2, replace_stride_with_dilation.at(0)));
    m_layer3 = register_module(
        "layer3",
        _make_layer(256, layers.at(2), 2, replace_stride_with_dilation.at(1)));
    m_layer4 = register_module(
        "layer4",
        _make_layer(512, layers.at(3), 2, replace_stride_with_dilation.at(2)));
  ...
  torch::nn::Sequential m_layer1{nullptr}, m_layer2{nullptr}, m_layer3{nullptr},
      m_layer4{nullptr};
  ...
  torch::nn::Sequential _make_layer(int64_t planes, int64_t blocks,
    ...
    layers->push_back(Block(m_inplanes, planes, stride, downsample, m_groups,
                            m_base_width, previous_dilation));
    m_inplanes = planes * Block::m_expansion;
    for (int64_t i = 0; i < blocks; i++) {
      layers->push_back(Block(m_inplanes, planes, 1, torch::nn::Sequential(),
                              m_groups, m_base_width, m_dilation));
    }
