...
template <typename T>
float measure_inference_latency(std::shared_ptr<T> model,
                                torch::Device& device,
                                const std::vector<int64_t>& input_size,
                                const int num_samples = 100, const int num_warmups = 10)
{
    ...
    float avg_latency = std::chrono::duration_cast<std::chrono::milliseconds>(end -
                                                                       begin)
                         .count() /
...
int main()
{
    ...
    float latency = measure_inference_latency(model, device, input_size, 100, 10);

    std::cout << "Inference Latency (BS = " << input_size.at(0) << "): "
              << std::setprecision(4) << latency
              << " [ms / image]" << std::endl;
